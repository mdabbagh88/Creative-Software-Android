...
public class AndroidLangCore extends LangCoreImplBase implements LangCore {
	...
	public void alert(Object... messages) {
		Toast.makeText(ApplicationContext.getContext(),
				getAlertString(messages), Toast.LENGTH_LONG).show();
	}
	...
	private void addLogMessage(String debugString) {
		...
		int trim = log.length() - 90 * THOUSAND;
		if (trim > 0)
			log.delete(0, trim);
	}
	...
	// private String createLogMessage(Throwable e, String title, String info,
	// Object... values) {
	// return createTraceString(e) + createLogMessage(title, info, values);
	// }

	@Override
	public void info(Object... values) {
		StackTraceElement element = new Throwable().getStackTrace()[2];
		String message = createLogMessage(INFO_TITLE, createLogString(element),
				values);
	...
	public void warn(Object... values) {
		StackTraceElement element = new Throwable().getStackTrace()[2];
		String message = createLogMessage(WARN_TITLE, createLogString(element),
				values);
